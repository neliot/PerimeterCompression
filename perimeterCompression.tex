...
The distance metric is based upon the mean of a set of agents and the standard deviation between those agent sets. The mean is calculated as shown in equation~\ref{eq:SwarmStabilityDistance2} where $\mu_d(S)$ is the mean. The standard deviation is calculated as shown in equation~\ref{eq:SwarmStabilityDistance3} where $\sigma_d(S)$ is the standard deviation. The mean value can be compared to the repulsion filed to see if the swarm is optimally distributed in that the mean value is as close to the repulsion filed as possible and the standard deviation identifies the overall differences in the distances which can cause the swarm agents to oscillate. A standard deviation of $\sigma_d(S) = 0$ would indicate that all the agents are equally spaced.
\small
\begin{equation}
