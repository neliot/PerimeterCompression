...
The cohesion component of the swarming effect ($p_{kc}$) is applied when an agent ($b$) and its neighbour ($b'$) are both within range ($C_b$). $\mathsf{ekc}(b,b')$ works in a similar manor to $\mathsf{ekr}(b,b')$. $\mathsf{ekc}(b,b')$ refrences the $p_{kr}$ matrix to obtain a weighting that is based upon the two agents status. If the agents are not both perimeter-based then the agents cohesion vector is scaled by the scalar value that is referenced in the matrix (Eq.~\ref{eq:scalarsFromMatrix}). e.g. If both agents are perimeter-based and the matrix is as shown in equation~\ref{eq:pkccoh} then the effect of the additional cohesion-compression weighting will be increase the generated cohesion-vector for that individual relationship (Eq.~\ref{eq:cohesion3}) which causes the agent to have a tendency to move towards that neighbour. 
