...
\title{A novel approach to flexible swarm structure control.}
\author[1,*]{Neil Eliot}
...
In this paper, we propose that the behaviour of an agent should be modified depending on whether or not it is on a \emph{perimeter}. Figure~\ref{fig:simplePerim2} shows a simple swarm with perimeters formed by agents in \textcolor{red}{red}. Perimeter-based agents can form part of an inner boundary or an outer boundary. The swarm can also contain non-perimeter agents which are shown in black.
...
Where \emph{i} represents an internal agent and \emph{p} is a perimeter agent. These are reflected in Figure~\ref{fig:simplePerim2} as:
...
When using Equation~\ref{eq:coh2} one arrayis used, $k_c$. This arrayis used to scale the cohesion vector generated between an agent pair which is proportional to their distance apart, which will be within $C$ as shown in Equation~\ref{eq:repulsion1a}. Consider the arrayshown in Equation~\ref{eq:kcexample1}.
