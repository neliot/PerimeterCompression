...
The effect of an agent being within this set is that it will generate a vector that should `encourage' an agent to maintain their proximity. When there are multiple neighbours the cohesion effects are cummulative as shown by equation~\ref{eq:cohesion2} which will create a vector towards the centre of its neighbours, creating a cohesive swarm. $\card{n_c(b)}$ denotes the cardinality of $n_c(b)$. This is the component of the overall vector calculation that has the $\kc$ quotient applied to it to allow the cohesion effect to be `balanced' with respect to other vector influences as described in ~\cite{eliot2017methods,eliot2018metric,eliot2019void}. 
...
The repulsion component of an agent's movement is calculated from interaction with its repulsion neighbours $n_r(b)$ (Eq.~\ref{eq:repulsion1a}) in a swarm ($\mathcal{S}$). Repulsion agents are identified as agents that are within the agent's ($b$) repulsion field ($\rb$).
...
In this paper, we propose that the behaviour of an agent should be modified depending on whether or not it is on a \emph{perimeter}. Figure~\ref{fig:simplePerim2} shows a simple swarm. Perimeter agents are highlighted in \textcolor{red}{red} and can form part of an inner or outer boundary. The swarm can also contain non-perimeter agents which are shown in black.
...
The polar angle with respect to $b$ of a neighbour, $b'$, $\pangle(b, b')$, is the counter-clockwise angle that vector $\vec{bb'} = b' - b$ makes with the positive $x$ axis, shown in Figure~\ref{fig:neighbours3} as $\alpha_i$ and defined by Equation~\ref{eq:pangle}.
...
= \prm(b')$, and 
$\kc$ is a $2\times 2$ boolean-indexed array of constants that determine the weight of the cohesion vector according to whether the interaction between $b,b'$ is between non-perimeter agents, non-perimeter\textrightarrow perimeter, perimeter\textrightarrow non-perimeter, or perimeter\textrightarrow perimeter agents.\\
...
\end{equation}
\normalsize
where $p_b = \prm(b)$, $p_{b'} = \prm(b')$, and $\rb$ is a 2x2 boolean-indexed array of constants that determine the radius of the \emph{repulsion field} for agents in the swarm, according to whether the interaction between $b,b'$ is between non-perimeter agents, non-perimeter\textrightarrow perimeter,
perimeter\textrightarrow non-perimeter, or perimeter\textrightarrow perimeter agents.\\
...
\end{equation}
\normalsize
where $p_b = \prm(b)$, $p_{b'} = \prm(b')$, and $\kr$ is a 2x2 boolean-indexed array of constants that determine the weight of a component of the repulsion vector according to whether the interaction between $b,b'$ is between non-perimeter agents, non-perimeter\textrightarrow perimeter, perimeter\textrightarrow non-perimeter, or perimeter\textrightarrow perimeter agents.
...
The resultant vector is simply the sum of the cohesion, repulsion and gap-filling vectors as shown in Equation~\ref{eq:res} and a resulant swarm segment is shown in Figure~\ref{fig:swarmExample}
...
The new modelling method allows for a highly configurable swarm. Each configuration has an impact on the swarm's structure. This is analysed using the inter-agent distances. The experimental results show the effects on the swarm in terms of inter-agent distances which are categorised based on the relationship status. The section will cover four experiments and will include a baseline swarm for comparison.

\subsection{Distance metric}
The distance metric is used by many researchers as a method of examining the structure of a swarm~\cite{BAFVM:06,BAF:06,elamvazhuthi2015optimal,VG:05,SW:03}. However, due to the new model allowing the field effects and magnitudes to be varied the distance metric will need to be adapted to analyse the agents involved in specific relationships rather than globally, therefore $S$ will be sub-divided into the three relationship categories of $S_i$, $S_p$, $S_{o}$. Where $S_i$ are the internal\textrightarrow internal relationships, $S_p$ are the perimeter\textrightarrow perimeter relationships and $S_{o}$ are all the internal\textrightarrow perimeter or perimeter\textrightarrow internal relationships. 
