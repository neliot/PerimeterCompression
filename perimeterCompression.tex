...
The compression effect on the cohesion of the perimeter agents is that any perimeter-perimeter relationships should be re-enforced using the cohesion compression quotient ($k_{cpc}$). This is achieved by splitting the cohesion agents into two subsets. One subset being the perimeter agents (Eq.\ref{eq:cohesion3}), the other being internal agents (Eq.\ref{eq:cohesion4}).
