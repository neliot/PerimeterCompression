...
\subsection{Perimeter repulsion ($p_r$, $\mathsf{erf}(b,b')$)}

The repulsion component of the compression effect is applied when an agent ($b$) and its neighbour ($b'$) are both perimeter-based. Thus the status of both agents in terms of $\mathsf{prm}(b)$ and $\mathsf{prm}(b')$ (true,true) will result in $\mathsf{erf}(b, b')$ return $d$ from the matrix. If this value is $<0$ then the effective field radii $R_b$ is reduced as shown in Equation~\ref{eq:prrep} where the repulsion field will be reduced by a $0.4$ weighting which effect a compression effect when combined with the cohesion modification as shown in figure~\ref{fig:perimeterCompression}.
