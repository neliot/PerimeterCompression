...
\section{Future Work}
Going forward the new model will be examined based upon the introduction of direction and obstacles. Initial testing shows that the models holds up well including the improvement in self-healing as demostrated in figure~????? which demonstrates an obstacle being introduced and removed in a packed perimeter swarm.
\begin{multicols}{2}
	\begin{figure}[H]
		\begin{center}
			\includegraphics[width=1.0\linewidth]{figures/FutureTime}
		\end{center}
		\caption{Packed self-healing\label{fig:packedSelfHealing}}
	\end{figure}
\end{multicols}
