...
The cohesion component of the swarming effect is applied when an agent ($b$) and its neighbour ($b'$) are both within range ($C_b$). $\mathsf{ekc}(b,b')$ works in a similar manor to $\mathsf{ekr}(b,b')$. $\mathsf{ekc}(b,b')$ refrences the $p_{kc}$ matrix to obtain a weighting that is based upon the two agents status. e.g. If both the agents are perimeter-based then the agents cohesion vector is scaled by the scalar value that is referenced in the matrix (Eq.~\ref{eq:scalarsFromMatrix}) i.e. using equation~\ref{eq:pkccoh} $p_{kc}[1][1]$ would return the value 500.0. 
